...
public class PlayerCommend {
    //call method when human player end turn
    public static void player1EndTurn(ActorRef out, GameState gameState) {
        BasicCommands.addPlayer1Notification(out, "end turn", 1);
        try {Thread.sleep(500);} catch (InterruptedException e) {e.printStackTrace();}
        drawCard(out, gameState);
        setPlayer1Mana(out, gameState,0);
        try {Thread.sleep(1000);} catch (InterruptedException e) {e.printStackTrace();}
    }
    //call method when AI player end turn
    public static void player1StartTurn(ActorRef out, GameState gameState) {
        gameState.turnRound++;
        BasicCommands.addPlayer1Notification(out, "Round " + gameState.turnRound + " start", 1);
        setPlayer1Mana(out, gameState,gameState.turnRound + 1);
    }
    // method for changing human player's health
    public static void setPlayer1Health(ActorRef out, GameState gameState , int health) {
        gameState.humanPlayer.setHealth(health);
        BasicCommands.setPlayer1Health(out, gameState.humanPlayer);
        gameState.checkWinorLose(out);
    }
    // method for changing human player's mana
    public static void setPlayer1Mana(ActorRef out, GameState gameState, int mana) {
        if(mana < 10) {
            gameState.humanPlayer.setMana(mana);
        } else {
            gameState.humanPlayer.setMana(9);
        }
    ...
    public static void playerSummonUnit(ActorRef out, GameState gameState,Tile tile) {
        ...
        setPlayer1Mana(out, gameState, gameState.humanPlayer.getMana() - gameState.selectedCard.getManacost());
    }
    ...
    public static void checkUnitStats(ActorRef out, GameState gameState, Unit unit, Tile tile) {
        ...
        if(unit instanceof HumanAvatar) {
            ...
            setPlayer1Health(out, gameState, unit.getHealth());
        } else if(unit instanceof AiAvatar) {
            AiPlayerCommend.setPlayer2Health(out, gameState, unit.getHealth());
        }
    }
